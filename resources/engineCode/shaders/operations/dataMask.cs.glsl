...
void main () {
	...
	const vec4 previousColorF = vec4( previousColor ) / 255.0;
	const vec3 previousLight = imageLoad( lightBlock, blockLocation ).xyz;

	bool doIt =
		( useR && abs( color.r - previousColorF.r ) < variances.r ) ||
		( useG && abs( color.g - previousColorF.g ) < variances.g ) ||
		( useB && abs( color.b - previousColorF.b ) < variances.b ) ||
		( useA && abs( color.a - previousColorF.a ) < variances.a ) ||
